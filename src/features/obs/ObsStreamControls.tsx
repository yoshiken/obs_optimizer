...
interface ObsStreamControlsProps {
  /** 録画コントロールを表示するか（デフォルト: false） */
 ...
 *
 * 配信の開始/停止ボタン、経過時間の表示
 * 配信特化ツールのため、録画機能はオプション扱い
 */
export function ObsStreamControls({ showRecording = false }: ObsStreamControlsProps = {}) {
  const {
    connectionState,
    status,
    loading,
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md dark:shadow-gray-900/50 p-6">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">
          {showRecording ? '配信・録画コントロール' : '配信コントロール'}
        </h3>
  ...
  return (
    <>
      ...
      {/* 録画完了通知 */}
      {showRecording && showRecordingPathNotice && lastRecordingPath && (
      ...
      <div className={showRecording ? 'grid grid-cols-2 gap-6' : 'max-w-md mx-auto'}>
        {/* 配信コントロール */}
